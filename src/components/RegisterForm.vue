<template>
  ...
  >
    ...
    <!-- Identity -->
    <div class="mb-3">
      <label class="inline-block mb-2">Identity</label>
      <vee-field
        as="select"
        name="identity"
        class="block w-full py-1.5 px-3 text-gray-800 border border-gray-300 transition duration-500 focus:outline-none focus:border-black rounded"
...
export default {
  ...
  data() {
    return {
      ...
      userData: {
        // 預設值的設定是跟tag裡的 value='xx' 中的'xx'值，和">'yy'<"不同，大小寫最好一樣。
        country: 'USA',
        identity: 'listener'
      },
  ...
  methods: {
    ...
    async register(values) {
      ...
      try {
        ...
        // console.log('register_success:', values)
      } catch (error) {
        // console.log('register_failed:', values)
